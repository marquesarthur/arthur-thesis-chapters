...
Since our main focus is on techniques able to infer the semantics of text, we further detail core concepts related to these techniques. 
...
That is, the model learns vector representations by \textit{(i)} looking at the $n$ words that preceed and succeed word $w_t$
...
Prediction relies on a mechanism, called \textit{attention} \red{ref}, that correlates and weights all non-replaced tokens in the input so that the model maximizes predictions.
BERT's training procedures make use of transfer learning  \red{ref}, i.e., 
