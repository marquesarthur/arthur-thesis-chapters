...
Figure~\ref{fig:eval-metrics-results} also shows results when applying sentence semantic filters to \texttt{word2vec} and \texttt{BERT}. 
...
the highest results ($0.53$ precision and $0.54$ recall). When paired with \texttt{BERT}, there are no noticeable changes to precision, but 
...
\red{Conclusion for the results}
...
% Table~\ref{tbl:approach-results-overall} reports pyramid precision and pyramid recall values for the VSM \texttt{baseline} as well as for the best configuration of the semantic techniques, i.e., .
