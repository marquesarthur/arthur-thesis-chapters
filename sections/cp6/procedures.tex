...
The plug-in takes a developer's task and a software artifact as inputs and then applies its underlying semantic-based technique the  to identify sentences that likely contain information useful to the input task.
The tool then highlights the text identified as relevant so that a developer can quickly located it amidst the rest of the content of an artifact  under inspection. 
...
We configure the semantic-based technique used by the study tool to identify no more than 10\% of the sentences of an artifact as relevant to the input task.
...
We gather input at the artifact level because it would be too exhausting for a participant 
to provide individual feedback on each highlight shown
in the time allotted for the experiment.
