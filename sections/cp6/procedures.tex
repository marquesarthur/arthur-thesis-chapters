...
Figure~\ref{fig:tool-experiment-procedures} presents an experiment designed to evaluate how a semantic-based tool might assists a software developer while working on a task.
...
a web browser plug-in that embeds one of our semantic based approaches, i.e., \texttt{BERT} with \textit{ no filters} (Chapter~\ref{ch:identifying}).


The plug-in takes a developer's task and a software artifact as inputs and then applies its underlying semantic-based technique the  to identify sentences that likely contain information useful to the input task,  highlighting the text found in the artifact under inspection. Based on our key findings on the portion of the text usually relevant to a task (Chapter~\ref{ch:characterizing}), we configure the study tool to identify no more than 10\% of the content of an artifact as relevant to the input task.
...
% In this section, we present an overview of the whole experiment and then, we provide 
% details related to the manual and tool-assisted tasks, respectively.
...
to identify sentences relevant to the input task. 
...
% a target number of sentences equal to 10\% of the entire content of an input artifact up to a maximum of 10 sentences per artifa
...
Participants rated highlights on a per artifact basis and,  before giving their answer, 
they could revisit an artifact and its highlights if so they wished. 
