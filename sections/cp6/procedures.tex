...
To determine how a tool embedding a semantic-based technique might impact developers while they work on a task, we question whether:



\begin{enumerate}
    \item \textit{a developer would produce a more correct solution for her task when assisted by such a tool;}
    \item \textit{in the software artifacts pertinent to a developer's task, the text automatically identified by the tool as relevant to is similar to text that she would deem relevant; and whether}
    \item \textit{in a certain artifact, a developer finds that the text automatically identified by the tool assists her towards task completion.}
\end{enumerate}


% (Section~\ref{cp5:approach-bert})
To investigate these questions, we designed an experiment where participants attempted two programming
 using (or not) a tool that applies a semantic-based technique 
 to  identify task-relevant text in the artifacts that were available for each task---highlighting the text automatically identified. 


For each task, we compare solutions  submitted by participants who did, or did not, use the tool to assist them while they performed that task. 
For that, we run a set of test cases that evaluate the correctness of the submitted solutions.


In the tasks that participants performed without tool support, we asked them to manually indicate what text that they deemed useful to the task at hand in the artifacts available for that task.
We use the manually provided text to investigate to which extent our tool identifies the same text for that task.
To compliment this analysis, in the tool-assisted task, we asked participants to indicate how useful were the highlights shown in each of the artifacts available.
We also allowed them to provide any additional feedback that they wished to share about the tool or the experiment. 
