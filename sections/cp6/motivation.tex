...
\gcm{automatically identifies
task-relevant text in pertinent
documents}  can affect a developer's work.
\gcm{Building on work presented earlier in this
thesis, the tool uses a semantic-based technique.}
\gcm{By identifying information} that is useful to the developer's task,
...
\gcm{If the tool is successful in identifying text useful to the task, we hypothesize that
the developer will be produce a correct solution more often than if they had not used the tool.}
...
\gcm{Even if a developer is more successful
