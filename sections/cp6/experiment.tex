...
\begin{enumerate}[label=\textit{RQ}\arabic*]
    \item Does the use of a semantic-based tool---\acs{tool}---help developers produce more correct solutions? \rev{With this question, we investigate whether there are any positive effects brought by \acs{tool}, i.e.,  if the use of our tool helps developers correctly complete a software task.}
    \item How useful is the text identified by \acs{tool}, i.e., does  automatically-identified text contain information that assists task completion? \rev{We seek to assess whether developers perceive the tool as useful, which is an important aspect for tool adoption.}
    \item How does the text automatically identified by \acs{tool} compare to text 
    that humans perceive as relevant to a task? \rev{We investigate if the text identified by \acs{tool} is aligned with the developers' perception of text relevant to a task. }
\end{enumerate}
...
\begin{figure}
    \centering
    \includegraphics[width=\dimexpr\linewidth-4\fboxsep-2\fboxrule]{cp6/tool-experiment-pipeline.pdf}
...
% \red{The first research question means comparing tool-assisted solutions against the solutions of a control group. The second research question focuses solely on the quality of the tool's output, i.e., the usefulness of the text identified. 
% Finally, the third research question considers the analytical comparison of the tool's output against data gathered with the control group.}
...
\rev{two randomly assigned tasks} drawn from a list with three well-known Python programming tasks.
Participants are asked to write a solution for \rev{a \textit{control} and a \textit{tool-assisted} task}.
\rev{Tasks configurations (control and tool-assisted) were equally balanced and }
...
\rev{the solutions for each of the task  described in the table requires} usage of different modules external to the Python core library. \rev{Although these modules may not be equivalent in difficulty}, their use reduces the likelihood that a participant 
...
While tasks were randomly assigned, \rev{we emphasize that the survey tool kept track of the number of times a task performed in the control and tool-assisted configurations.}
...
its underlying semantic-based technique identified as relevant to the task\footnote{
    We configure the tool to identify no more than 10\% of the sentences of an input artifact as relevant \rev{with regards to an input task title}.
