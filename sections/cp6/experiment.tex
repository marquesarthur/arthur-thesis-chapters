...
\begin{enumerate}[label=\textit{RQ}\arabic*]
    ...
    \item How useful is the text identified by \acs{tool}, i.e., does the automatically-identified text contain information that assists task completion? \rev{We seek to assess whether developers perceive the tool as useful, which is an important aspect of tool adoption.}
    \item How does the text automatically identified by \acs{tool} compare to text 
    that humans perceive as relevant to a task? \rev{We investigate if the text identified by \acs{tool} aligns with the developers' perception of text relevant to a task. }
\end{enumerate}
