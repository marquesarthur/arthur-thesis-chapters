...
we compile their submitted code and run it against a set of 10 test cases \gcm{(not provided to participants)}that check whether it produces the correct output for each given test input. 
...
we cannot draw statistically significant conclusions based on the correctness scores of each group~\cite{Lazar2017-cp3}. \gcm{You should provide
the statistical test and why its not valid.}
Hence, we evaluate scores on a per-task basis, as shown in Figure~\ref{fig:correctness-by-task}. \gcm{Probably need to say something
as well about why there isn't enough power to do statistical tests.}
