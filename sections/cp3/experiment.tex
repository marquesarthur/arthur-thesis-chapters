...
\rev{Table~\ref{tbl:cp3-experiment-overview} summarizes our design.
The main independent variable for this experiment is the information seeking task provided
to a participant. We assigned the tasks to participants in a randomized order.
The dependent variable is the highlights produced
by a participant.}
This design allows us to gather both 
quantitative data (in the form of the highlights produced) 
and qualitative data (in the form of the interview transcripts about the highlights) 
...
\input{sections/cp3/tbl-experimental-design.tex}
...
\input{sections/cp3/tbl-assumption-examples.tex}
...
Table~\ref{tbl:assumptions} shows examples of these criteria in action, the \texttt{Lucene}
artifacts chosen contain
complementary sources where sentences in different artifacts
pertain to the same topic (\rev{i.e., how the length of an document affects Lucene's BM25 similarity function})
...
sentences and the average reading time\footnote{\rev{Based on the average readers' reading speed of 200 words per minute~\cite{Just1980}}} of all artifacts of each task
...
experiment's purpose, i.e., \rev{determine what text developers deem relevant to a particular software task}.
\rev{Then, as a means to identify the task-relevant text,
participants were provided with a tool (in
the form of a browser extension) that allowed them to 
highlight text they considered relevant to their assigned tasks.}
