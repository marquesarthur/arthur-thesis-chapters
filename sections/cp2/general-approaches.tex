...
\acf{IR} comprises \rev{the structure, analysis, storage, search, and retrieval of information~\cite{salton1983} and \acs{IR} research has many research fields~\cite{zhang2016, liu2019data, tamine2010}, including: 
\textit{ad-hoc retrieval}, or the basis upon which an IR system strives to return an accurate set of documents relevant to an input query;
\textit{question answering}, or finding answers to an input question; and
\textit{contextual retrieval}, or finding information based on a particular context, i.e., the user's task. Among these research fields, the work in this thesis is closer to contextual retrieval, which we further detail it in the remainder of this section.}


\rev{Context consists of many factors such as a user's intentions, tasks, and needs. Such factors are often modeled as attributes that describe the user and the environment in which information activities occur~\cite{dourish2004we}. For example, a contextual-aware \acs{IR} system could provide different recommendations based on a user's age
to find documents appropriate to a particular age group~\cite{kofod2003case}.} 




\rev{A key factor of interest to this work is `task context', i.e., the information available in a software task which is assigned to a software developer. This information, or a subset of it, can serve as a starting point to an input query, where} classical \acs{IR} systems use the frequency or co-occurrence of words (or phrases) to determine the relevance of an artifact for that input query.
...
or how to account for different words that appear interchangeably in some context (\textit{LSI})~\cite{dumais1994latent}. \rev{We discuss other and more complex schemes, which use \acf{NLP}, \acf{ML}, or \acf{DL} to identify relevant text, in subsequent sections of this chapter.}
