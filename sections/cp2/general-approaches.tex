...
\acf{IR} comprises \rev{the structure, analysis, storage, search, and retrieval of information~\cite{salton1983}. Such a definition encampsulates }
techniques
or approaches for finding entities 
that satisfy an information need (documents, paragraphs, sentences, etc.)~\cite{manning2010IR}.


\rev{\acs{IR} research has many subfields~\cite{zhang2016, liu2019data, tamine2010}, including
...
\textit{contextual retrieval}, or finding information based on a particular context, i.e., the user's task.
These research fields often intersect with other fields, such as \acf{NLP}, \acf{ML}, or \acf{DL}, and given the wide scope of  \acs{IR} research, this section focuses on early concepts associated with search-related  \acs{IR}. We discusse IR efforts in other fronts in subsequent sections of this chapter. } 


 

\rev{Early} \acs{IR} \rev{techniques}
use the frequency or co-occurrence of words (or phrases) to determine the relevance
