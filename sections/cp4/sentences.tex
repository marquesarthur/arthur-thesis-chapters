...
Overall, annotation required a total of $\approx54$ hours of manual work and it was done throughout the course of 5 weeks.
...
8.93 sentences. \gm{\textbf{\textit{per artifact?}}} \art{yes}
...
dilute \gm{\textbf{\textit{dilute?}}} \art{ops will rewrite} and exemplify much of the information that a developer would find in official API documentation in a more didactic way~\cite{arya2020, Jiang2017}.
For the latter, the content on GitHub may be too project-specific what might 
prevent a developer performing a task in a different context from benefiting from the project-specific information.
...
We use a Wilcoxon-Mann-Whitney test~\cite{mannWhitneyU} to check if there are any differences between the number of sentences marked in 
GitHub and Stack Overflow task.
Results show a statistically significant difference 
between the number of sentences 
marked with an effect size (\textit{Cohen's d = 0.29})~\cite{grissom2005effect} in between the small and medium thresholds~\cite{cohen2013statistical}.
Based on this analysis, evaluation should provide results considering
